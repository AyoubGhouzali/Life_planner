...
import { getWeeklySummary, getLifeBalanceScores } from "@/lib/db/queries/analytics";
    ...
    <div className="flex-1 space-y-6 p-4 md:p-8 pt-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{summary.completedTasks.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{summary.overdueTasks.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {summary.habitStats.length > 0 
                ? (summary.habitStats.reduce((acc, curr) => acc + curr.count, 0) / summary.habitStats.length).toFixed(1)
                : 0}
            </div>
            <p className="text-xs text-muted-foreground">Avg completions per habit</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="col-span-4 space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Great job finishing these tasks!</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {summary.completedTasks.slice(0, 10).map((task) => (
                  <div key={task.id} className="flex items-center justify-between border-b pb-2 last:border-0">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{task.areaName}</p>
                    </div>
              ...
              <div className="space-y-4">
                {summary.overdueTasks.map((task) => (
                  <div key={task.id} className="flex items-center justify-between border-b pb-2 last:border-0">
                    ...
                    <p className="text-xs text-muted-foreground">{task.areaName}</p>
                  </div>
                ))}

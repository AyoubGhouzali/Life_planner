...
export const boards = pgTable("boards", {
  id: uuid("id").defaultRandom().primaryKey(),
  area_id: uuid("area_id").references(() => lifeAreas.id, { onDelete: "cascade" }).notNull(),
  name: text("name").notNull(),
  description: text("description"),
  position: integer("position").default(0).notNull(),
  created_at: timestamp("created_at").defaultNow().notNull(),
...
export const columns = pgTable("columns", {
  id: uuid("id").defaultRandom().primaryKey(),
  board_id: uuid("board_id").references(() => boards.id, { onDelete: "cascade" }).notNull(),
  name: text("name").notNull(),
  color: text("color"),
  position: integer("position").default(0).notNull(),
...
export const projects = pgTable("projects", {
  id: uuid("id").defaultRandom().primaryKey(),
  column_id: uuid("column_id").references(() => columns.id, { onDelete: "cascade" }).notNull(),
...
export const habits = pgTable("habits", {
  id: uuid("id").defaultRandom().primaryKey(),
  user_id: uuid("user_id").references(() => profiles.id, { onDelete: "cascade" }).notNull(),
  area_id: uuid("area_id").references(() => lifeAreas.id, { onDelete: "set null" }),
  name: text("name").notNull(),
  description: text("description"),
...
export const lifeAreasRelations = relations(lifeAreas, ({ one, many }) => ({
  user: one(profiles, { fields: [lifeAreas.user_id], references: [profiles.id] }),
  boards: many(boards),
}));

export const boardsRelations = relations(boards, ({ one, many }) => ({
  area: one(lifeAreas, { fields: [boards.area_id], references: [lifeAreas.id] }),
...
export const projectsRelations = relations(projects, ({ one, many }) => ({
  column: one(columns, { fields: [projects.column_id], references: [columns.id] }),
  tasks: many(tasks),
  notes: many(notes),
...
export const habitsRelations = relations(habits, ({ one, many }) => ({
  ...
  area: one(lifeAreas, { fields: [habits.area_id], references: [lifeAreas.id] }),
  logs: many(habitLogs),
}));

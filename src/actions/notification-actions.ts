...
export async function generateNotifications(userId: string) {
  try {
    ...
    for (const { task } of allTasks) {
        ...
        const isDueToday = dueDate >= todayStart && dueDate < new Date(todayStart.getTime() + 24 * 60 * 60 * 1000);
        ...
        const hasNotificationToday = existingNotifications.some(n => 
            n.type === 'overdue' && n.link === `/tasks/${task.id}` && n.created_at >= todayStart
        );
        ...
        const hasNotification = existingNotifications.some(n => 
            n.type === 'due_today' && n.link === `/tasks/${task.id}`
        );
